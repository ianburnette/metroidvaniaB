...
public class ClassicRpgGui : MonoBehaviour {
	...
	public Sprite img_mohaned;
	public Sprite img_mom;
	
	public float ScreenWidthModifier = 0.5f;
	public float ScreenHeightModifier = 100f;
	public float BoxWidthModifier = 512f;
	public float BoxHeightModifier = 190f;
		
	public float nameXmod = 0;
	public float nameYmod = 60;
	public float nameWidthMod = 150;
	public float nameHeightMod = 50;
	...
	void Update () {
		...
		if(!_isBranchedText){
			if(Input.GetMouseButtonDown(0) || Input.GetButtonDown("Interact") || Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.Return)){
			...
			if(Input.GetButtonDown("Interact") || Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.Return)){
				if(_windowCurrentText == _windowTargetText){
	...
	void CheckToDisplayBox(){
		if (_showDialogueBox && !dialoguePanel.gameObject.activeSelf){
			dialoguePanel.gameObject.SetActive(true);
		}else if (!_showDialogueBox && dialoguePanel.gameObject.activeSelf){
	...
	void DisplayPortrait(string name){
		if (name == "MOHANED"){
			portraitBox.sprite = img_mohaned;
		}else if (name == "MOM"){
			portraitBox.sprite = img_mom;
		}
		else{
			print ("error - incorrect name: " + name);
		}
	}
	...
	void OnGUI(){
		...
		float rectX = (!_usingPositionRect) ? Screen.width*ScreenWidthModifier : _positionRect.x;
		float rectY = (!_usingPositionRect) ? Screen.height - ScreenHeightModifier : _positionRect.y;
		float rectWidth = (!_usingPositionRect) ? BoxWidthModifier : _positionRect.width;
		float rectHeight = (!_usingPositionRect) ? BoxHeightModifier : _positionRect.height;
		...
		if(_nameText != string.Empty){
			Rect nameBoxRect = new Rect(dialogueBoxRect.x + nameXmod, dialogueBoxRect.y - nameYmod, nameWidthMod * _windowTweenValue, nameHeightMod * _windowTweenValue);
